class sudoku_board():
    ...
    def check_sudoku(self, digit, row, col):
        ...
        if (self.check_rows(digit, row) and self.check_columns(digit, col) and self.check_segment(digit, row, col)):
            return True
        else:
            return False
    ...
    def check_segment(self, digit, row, col):
        i = row - row % 3
        j = col - col % 3
        nums = self.board[i][j:j + 3] + self.board[i + 1][j:j + 3] + self.board[i + 2][j:j + 3]     
            ...
            for col in range (0, 9): # iterating over columns
                ...
                if (self.board[row][col] == 0): # checking if board[row][column] is 0 
                    for digit in range(1, 10): # loop over possible values from 1 to 9
                        ...
                        if(self.check_sudoku(digit, row, col) == True): # check if value "digit" from loop above is correct
                            
                            self.board[row][col] = digit # if value "digit" is correct, try to put value in board[row][col]

                            if (self.solver() == True): # backtrack for another field
                                return True
                            else:
                                self.board[row][col] = 0 # if solution is wrong, back to previous empty field
